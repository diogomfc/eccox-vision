...
import { Layers, Check, User } from "lucide-react";
...
export default function MachineReviewStep({
  ...
  const getServerImage = () => {
    switch (machine.status) {
      case "Concluída":
        return ImgServerConcluida;
      case "Pendente":
        return ImgServerPendente;
      default:
        return ImgServerNew;
    }
  };
  ...
  const completedServices = applications.reduce(
    (acc, app) =>
      acc + app.services.filter((s) => s.status === "Concluída").length,
    0
  );
  const pendingServices = applications.reduce(
    (acc, app) =>
      acc + app.services.filter((s) => s.status === "Pendente").length,
    0
  );
  ...
  return (
    ...
    >
      ...
      <div
        className={`bg-gradient-to-br from-[#1A1A1D] to-[#0F0F11] border ${
          isEditMode ? "border-amber-500/30" : "border-blue-500/30"
        } rounded-lg p-6`}
      >
          ...
          <div className="flex-shrink-0 flex items-center gap-6">
            ...
            <div className="relative flex items-center justify-center">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span
                  className={`text-2xl font-bold ${themeConfig.progressBg}`}
                >
                  {progressPercentage}%
                </span>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-gray-700/50">
          <div className="text-center">
            ...
            <p className="text-gray-200 font-medium">
              {machine.updatedAt
                ? format(new Date(machine.updatedAt), "dd/MM/yyyy", {
                    locale: ptBR,
                  })
                : "Não definida"}
            </p>
      ...
      {/* Resumo Final */}
      <div
        className={`${themeConfig.bgAccent} border ${themeConfig.borderAccent} rounded-lg p-4`}
      >
        <div className="flex items-start gap-3">
          ...
          <div>
            <h4
              className={`text-sm font-medium ${themeConfig.accentColor} mb-1`}
            >
