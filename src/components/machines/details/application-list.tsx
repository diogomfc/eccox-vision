...
import { Layers, SquarePen, Trash2, Plus, Loader2, User } from "lucide-react";
...
export function ApplicationList({
  ...
  const handleCreateApplication = useCallback(
    async (newApplication: AppType) => {
      ...
      try {
        ...
        const result = await window.electronAPI.syncApplication(
          applicationToCreate
        );
  ...
  const handleUpdateApplication = useCallback(
    async (updatedApplication: AppType) => {
      setIsLoading(true);
      setMessage(null);
  ...
  const handleDeleteApplication = useCallback(
    async (applicationId: string) => {
      ...
      try {
        ...
        const result = await window.electronAPI.deleteApplication(
          applicationId
        );
          ...
          return (
            <li
              ...
              className={`mb-2 cursor-pointer rounded p-2 hover:bg-[#23232B] transition ${
                selectedApp === app.name ? "bg-[#23232B] border border-blue-500" : ""
            ...
            >
              <HoverCard>
                <HoverCardTrigger asChild>
                  <div className="p-2 hover:bg-[#23232B] rounded">
                    ...
                    <div className="flex items-center gap-2 pr-0">
                      <div className={`flex-1 h-2 rounded-full overflow-hidden ${app.percent === 0 ? 'bg-red-900/20' : 'bg-[#2f2f36]'}`}>
                        <div
                ...
                <HoverCardContent className="w-80 bg-[#23232B] text-white border border-[#333] p-4 rounded-lg shadow-lg z-20">
                  <div className="space-y-2">
                    ...
                    <div className="pt-2 border-t border-gray-600/50 gap-1 flex flex-col">
                      <p className="text-xs text-gray-400">
                        Total de Serviços:{" "}
                        <span className="font-semibold text-white">
                          {app.services.length}
              ...
              {hoveredAppId === app.name && !isLoading && (
                <motion.div
                  ...
                  className="absolute top-1/2 right-2 transform -translate-y-1/2 flex flex-col gap-1 z-10 bg-[#23232B] p-1 rounded"
                >
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
