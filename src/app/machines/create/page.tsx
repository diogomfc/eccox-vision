              ...
              <CardTitle className="flex items-center gap-3 text-xl">
                ...
                <div>
                  <h2 className="text-xl font-semibold text-gray-100">
                    {currentStep?.title || "Passo"}
                  </h2>
                  <p className="text-sm text-gray-400 font-normal mt-1">
                    {activeStep === 1 && "Configure os dados básicos da sua máquina mainframe"}
                    {activeStep === 2 && "Adicione e configure as aplicações e serviços"}
                    {activeStep === 3 && "Revise todos os dados antes de criar a máquina"}
                  </p>
                </div>
              </CardTitle>
            </CardHeader>
                ...
                <AnimatePresence mode="wait">
                  ...
                  {activeStep === 3 && (
                    <motion.div
                      ...
                      className="space-y-6"
                    >
                      <div className="bg-[#0F0F11] p-4 rounded-lg border border-[#1F1F23]">
                        <h4 className="font-medium text-gray-100 mb-3">Informações da Máquina</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                          <div>
                            <span className="text-gray-400">Nome:</span>
                            <span className="ml-2 text-gray-100">{newMachine.name}</span>
                          </div>
                          <div>
                            <span className="text-gray-400">Versão:</span>
                            <span className="ml-2 text-gray-100">{newMachine.version}</span>
                          </div>
                          <div>
                            <span className="text-gray-400">Status:</span>
                            <Badge variant="outline" className={`ml-2 ${getStatusBadgeColor(newMachine.status || '')}`}>
                              {newMachine.status}
                            </Badge>
                          </div>
                          <div>
                            ...
                            <span className="ml-2 text-gray-100">
                              {newMachine.updatedAt
                                ? format(new Date(newMachine.updatedAt), "PPP", { locale: ptBR })
                                : "Não definida"
                              }
                            </span>
                          </div>
                          <div className="md:col-span-2">
                            ...
                            <span className="ml-2 text-gray-100">{newMachine.description}</span>
                          </div>
                        </div>
                      </div>
                      <div className="bg-[#0F0F11] p-4 rounded-lg border border-[#1F1F23]">
                        ...
                        {applications.length === 0 ? (
                          <p className="text-gray-400 text-sm">Nenhuma aplicação adicionada</p>
                        ) : (
                          <div className="space-y-3">
                            {applications.map((app) => (
                              <div key={app.id} className="border border-[#2A2A2D] rounded p-3">
                                <div className="flex items-center justify-between mb-2">
                                  <h5 className="font-medium text-gray-100">{app.name}</h5>
                                  <div className="flex items-center gap-2">
                                    <Badge variant="outline" className={getStatusBadgeColor(app.status)}>
                                      {app.status}
                                    </Badge>
                                ...
                                <p className="text-sm text-gray-400 mb-2">Tipo: {app.tipo}</p>
                                {app.services.length > 0 && (
                                  <div className="space-y-1">
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                      {app.services.map((service) => (
                                        <span
                                          key={service.id}
                                          className="text-xs bg-[#1A1A1D] px-2 py-1 rounded text-gray-300"
                                        >
                                          {service.name}
                                        </span>
                                      ))}
